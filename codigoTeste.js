...
function extrairDadosRastreio(html) {
    const eventos = [];
    let objetoPostado = false;
    let objetoEntregue = false;
    ...
    html(".linha_status").each(function () {
        ...
        if (status.includes("Objeto postado")) {
            objetoPostado = true;
            eventos.push({ status, data });
        } else if (status.includes("Objeto entregue ao destinatário")) {
            objetoEntregue = true;
        }
    });

    // Se o objeto foi postado e entregue, retorna um array vazio.
    // Isso indica que não precisamos salvar esse código.
    if (objetoPostado && objetoEntregue) {
        return [];
    }

    // Caso contrário, retorna os eventos de postagem.
    return eventos;
}
...
//     }
// }
// Substitua com seus códigos de rastreio para teste
const codigoTesteNaoEntregue = "NB998264352BR"; // Substitua com um código que ainda não foi entregue
const codigoTesteEntregue = "NB997952267BR";   // Substitua com um código que já foi entregue

main(codigoTesteNaoEntregue, codigoTesteEntregue);

async function main(codigoTesteNaoEntregue, codigoTesteEntregue) {
    console.log("Iniciando testes com códigos específicos...");

    // Testa o código não entregue
    if (codigoTesteNaoEntregue) {
        const resultadoTesteNaoEntregue = await rastrearObjeto(codigoTesteNaoEntregue);
        console.log(`Resultado do teste para código não entregue (${codigoTesteNaoEntregue}):`, resultadoTesteNaoEntregue);
    }

    // Testa o código entregue
    if (codigoTesteEntregue) {
        const resultadoTesteEntregue = await rastrearObjeto(codigoTesteEntregue);
        console.log(`Resultado do teste para código entregue (${codigoTesteEntregue}):`, resultadoTesteEntregue);
    }

    // Continua com a execução normal do script...
    // ...
}
